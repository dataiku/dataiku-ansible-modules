...
def get_client_from_parsed_args(module):
    ...
    api_key = args.api_key if args.api_key is not None else args.connect_to.get("api_key", os.environ.get("DATAIKU_ANSIBLE_DSS_API_KEY", None))
    if api_key is None:
        module.fail_json(msg="Missing an API Key, either from 'api_key' parameter, 'connect_to' parameter or DATAIKU_ANSIBLE_DSS_API_KEY env var".format(args.state))
    port = args.port if args.port is not None else args.connect_to.get("port", os.environ.get("DATAIKU_ANSIBLE_DSS_PORT", "80"))
    host = args.host 
